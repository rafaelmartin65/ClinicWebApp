...
public class PacienteDaoImple implements PacienteDao {
	...
	public boolean update(Paciente pac) {
		return jdbcTemplate.update("UPDATE PACIENTES SET nombre=:nombre, apellido=:apellido, edad=:edad, telf=:telf, historial=:historial WHERE idPaciente=:idPaciente", new BeanPropertySqlParameterSource(pac)) == 1;
	}

	@Override
	public boolean delete(int idPaciente) {
		// TODO Auto-generated method stub
		return jdbcTemplate.update("delete from Pacientes where idPaciente=:idPaciente", new MapSqlParameterSource("idPaciente", idPaciente)) == 1;
	}
	...
	public int[] saveAll(List<Paciente> pacientes) {
		...
		return jdbcTemplate.batchUpdate("insert into Pacientes (idPaciente, nombre, apellido, edad, telf, historial)"
				+ "values (:idPaciente, :nombre, :apellido, :edad, :telefono, :historial)", batchArgs);
	}
